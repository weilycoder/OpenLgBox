...
class Record(object):
    def __init__(self, data: list[str], contests: list[tuple[str, int]]):
        contest_id = int(data[0])
        self.contest, self.year = contests[contest_id] if contest_id < len(contests) else (str(contest_id), 0)
...
class Oier(object):
    def __init__(self, data: list[str], contests: list[tuple[str, int]]):
        ...
        self.records = [
            Record(record.strip().replace(";", ":").split(":"), contests)
...
class OierDB(object):
    def __init__(
        ...
        self.contests: list[tuple[str, int]] = []
        if contest_file is not None:
            with open(contest_file, "r", encoding="utf-8") as f:
                ...
                for contest in contest_json:
                    self.contests.append((contest["type"], contest["year"]))
        self.contest_names = {name for name, _ in self.contests}
